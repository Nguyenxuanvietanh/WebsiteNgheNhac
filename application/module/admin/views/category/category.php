<?php
    ...
    // echo '</pre>';
    $rowID = '';
    if(isset($formData['idtheloai'])){
        $inputID	= '<input type="text" name="form[idtheloai]" id="idtheloai" value="'.$formData['idtheloai'].'" class="form-control" readonly>';
        $rowID		= Helper::cmsFormRow('ID', $inputID);
    }
    //Input
    $inputCategory         = Helper::cmsInput('text', 'form[tentheloai]', 'tentheloai', null, 'form-control required');
    if(isset($formData['tentheloai'])){
        $inputCategory         = Helper::cmsInput('text', 'form[tentheloai]', 'tentheloai', $formData['tentheloai'], 'form-control required');
    }
    
    $inputHinhAnh       = Helper::cmsInput('file', 'form[hinhanh]', 'hinhanh', null, 'required');

    $rowCategory        = Helper::cmsFormRow('Tên Thể Loại', $inputCategory, true);
    ...
    <div class="main">
      <form action="#" id="adminForm" name="adminForm" method="POST" enctype="multipart/form-data">
              ...
              <tbody>
                ...
                <?php
                    ...
                    foreach($this->categories as $category){
                        $row = ($i % 2 == 0) ? 'row-chan' : ' ';
                        $id = $category['idtheloai'];
                        $image = '<img width="200" height="50" src="public/images/categories/'.$category['hinhanh'].'" alt="">';
                        $link = URL::CreateLink('admin', 'category', 'index', array('idtheloai' => $id));
                        echo '<tr class="'.$row.'">
                            <td>
                                <input type="checkbox" name="cid[]" id="'.$id.'" value="'.$id.'" class="chcktbl" />
                                <label for="'.$id.'"></label></td>
                            <td>'.$image.'</td>
                            <td><a href="'.$link.'">'.$category['tentheloai'].'</a></td>
                            <td>'.$id.'</td>
                        </tr>';
                        $i++;
                    }
          ...
          <div class="category-form">
            ...
            <?php echo $rowID . $rowHinhanh . $rowCategory . $inputToken; ?>
          </div>
      </form>
    </div>
